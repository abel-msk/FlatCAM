...
def gettext(text: str) -> str:
    return translation_dict.get(text, text) if sys.platform == 'win32' else gt.gettext(text)
    ...
    try:
        ...
        if sys.platform == 'win32':
            ...
            with resources.as_file(resources.files(SEARCH_PATH)) as local_path:
                new_lang = gt.translation('strings', localedir=local_path, languages=[lang_code])
                new_lang.install()
    except:
